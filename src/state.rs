...
impl State {
    pub async fn new(opts: Args) -> BoxResult<Self> {
        ...
        let url = opts.url.parse().expect("Could not parse url");
    ...
    pub async fn get_metrics(&self) -> Result<(), RestError> {
        ...
        //            ];
        //            log::debug!("Adding metric: elastic_billing_daily_cost_total, labels: {:?}, value: {}", &labels, cluster.value.clone());
        ...
        for deployment in &deployments_day.deployments {
            ...
            log::debug!(
                "Adding metric: elastic_billing_daily_cost_total, labels: {:?}, value: {}",
                &labels,
                deployment.costs.total.clone()
            );
            metrics::gauge!(
                "elastic_billing_daily_cost_total",
                deployment.costs.total.clone(),
                &labels
            );

            log::debug!(
                "Adding metric: elastic_billing_hourly_rate, labels: {:?}, value: {}",
                &labels,
                deployment.hourly_rate.clone()
            );
            metrics::gauge!(
                "elastic_billing_hourly_rate",
                deployment.hourly_rate.clone(),
                &labels
            );
            ...
            for item in &deployment.costs.dimensions {
                ...
                metrics::gauge!(
                    "elastic_billing_itemized_daily_cost_total",
                    item.cost.clone(),
                    &labels
                );
            }
        ...
        for deployment in &deployments_month.deployments {
            ...
            log::debug!(
                "Adding metric: elastic_billing_monthly_cost_total, labels: {:?}, value: {}",
                &labels,
                deployment.costs.total.clone()
            );
            metrics::gauge!(
                "elastic_billing_monthly_cost_total",
                deployment.costs.total.clone(),
                &labels
            );

            log::debug!(
                "Adding metric: elastic_billing_monthly_hourly_rate, labels: {:?}, value: {}",
                &labels,
                deployment.hourly_rate.clone()
            );
            metrics::gauge!(
                "elastic_billing_monthly_hourly_rate",
                deployment.hourly_rate.clone(),
                &labels
            );
            ...
            for item in &deployment.costs.dimensions {
                ...
                metrics::gauge!(
                    "elastic_billing_itemized_monthly_cost_total",
                    item.cost.clone(),
                    &labels
                );
            }
